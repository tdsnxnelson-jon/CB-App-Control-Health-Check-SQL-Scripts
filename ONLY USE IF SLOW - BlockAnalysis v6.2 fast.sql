...
/* ---------------------------------------------------------------------------
 BlockAnalysis v6.2 fast

 Identical output columns to "BlockAnalysis v6.2.sql", but the discovery-event
 OUTER APPLY (a scan of dbo.events by host_id + antibody_id) is removed. Use
 this on servers where v6.2 runs too long.

 What you lose vs. v6.2 - these columns are returned empty/NULL:
   Subtype, DiscoveryTimeStamp, DiscoveredBy, ProcessShort, ProcessFull
   (all four came from the "New unapproved file to computer" event)
 and RuleName can no longer produce 'Remote Writing Local', because that
 classification needs the process that originally wrote the file. Those rows
 fall through to the normal Approved/Unapproved classification instead.

 Block counts, hosts, paths, publishers, root-file data and BlockOutcome are
 unaffected - this is purely a metadata reduction.
--------------------------------------------------------------------------- */
...
SELECT
  ...
  CAST(NULL AS datetime) AS 'DiscoveryTimeStamp',
